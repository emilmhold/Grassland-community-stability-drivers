...
no.spp <- read_rds("output/richness23.rds") %>%
  dplyr::select(Plot, richness, AB.biomass, Productivity)
str(no.spp)
...
social.productivity.lmm <- lmer(AB.biomass ~ total.pos.cooccurrence*total.neg.cooccurrence + (1|Block), data = gain.loss)
summary(social.productivity.lmm)
Anova(social.productivity.lmm)
qqnorm(resid(social.productivity.lmm))
qqline(resid(social.productivity.lmm))
shapiro.test(resid(social.productivity.lmm)) 
...
##productivity
greg.productivity.plot <- ggplot(cooccurrence.forplot, aes(x = Gregariousness, y = Productivity)) +
  geom_point() + 
