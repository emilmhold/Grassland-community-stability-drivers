...
gain.loss <- read_rds("output/bcd matrix.rds") %>% #import temporal beta diversity df
  ...
  mutate(across(c(total.pos.cooccurrence, total.neg.cooccurrence), ~ scale(.x, center = TRUE, scale = FALSE))) ## scale social metrics
str(gain.loss)
## export data
...
title <- ggdraw() + 
  draw_label("Figure 2", x = 0, hjust = 0, size = 20)

final.social.plot <- plot_grid(
  title,
  social.plots,
  ncol = 1,
...
ggsave(filename = "social metric plots.jpeg", 
       final.social.plot,
       path = "figures/",
       width = 12,
       height = 15,
