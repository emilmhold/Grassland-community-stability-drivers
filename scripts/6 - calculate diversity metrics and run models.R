...
fdiv <- read_rds("output/July 2021 FD.rds") %>%
  dplyr::select(Plot, FRic, FDiv, FEve)
str(fdiv)
...
diversity.df <- no.spp %>%
  ...
  mutate(across(c(richness21, FDiv), ~ scale(.x, center = TRUE, scale = FALSE))) ##scale data
str(diversity.df)

#### models ####
## losses models
div.losses.lmm <- lmer(spp.losses ~ richness21*FDiv + (1|Block), data = diversity.df)
...
## gains models
div.gains.lmm <- lmer(spp.gains ~ richness21*FDiv + (1|Block), data = diversity.df)
