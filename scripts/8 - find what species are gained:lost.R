...
#### calculate proportion of total abundance represented by the most common species ####
## for 2021
sum(spp_summary$Total.abundance)/sum(kin21_total.abundance$Total.abundance)

## for 2023
##2023 data as abundance, not pa
kin23_total.abundance <- read.csv("data/July 2023 % cover.csv") %>%
  mutate_at(c(7:84), as.numeric) %>%
  dplyr::select(!(c(Date.Collected, Waypoint, Block, Plot.1, Treatment, Bare.Ground, Litter, Cow.pat, Rock))) %>%
  dplyr::select(!(starts_with("Unk"))) %>%
  rename(Rub.ida2 = Arb.ida) %>%
  ...
  rownames_to_column(var = "Spp") %>%
  rename(Total.abundance = ".")
str(kin23_total.abundance)

#2023 abundance for most common spp
spp_summary_23 <- kin23_total.abundance %>%
  filter(Spp %in% spp_summary$Spp)

##find proportion
sum(spp_summary_23$Total.abundance)/sum(kin23_total.abundance$Total.abundance)

## make a list of the species in the network
network.species <- c("Ach.mil", "Ane.pat", "Art.fri", "Art.lud", "Ast.das", "Ast.fal", "Ast.lae",
             "Cam.rot", "Carex", "Com.umb", "Dan.int", "Ely.tra", "Fes.hal", "Fra.vir",
             "Gal.bor", "Lat.och", "Pas.smi", "Poa.pra", "Pot.arg", "Ros.ark", "Sol.mis",
             "Sti.cur", "The.rho", "Tar.off", "Vic.ame")

## 2021 species
network_spp_summary_21 <- kin21_total.abundance %>%
  filter(Spp %in% network.species)

##find proportion
sum(network_spp_summary_21$Total.abundance)/sum(kin21_total.abundance$Total.abundance)

## 2023 species
network_spp_summary_23 <- kin23_total.abundance %>%
  filter(Spp %in% network.species)
##find proportion
sum(network_spp_summary_23$Total.abundance)/sum(kin23_total.abundance$Total.abundance)
