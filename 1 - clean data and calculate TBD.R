...
kin23 <- read.csv("data/July 2023 % cover.csv") %>%
  ...
  dplyr::select(!(c(Date.Collected, Waypoint, Block, Plot.1, Treatment, Bare.Ground, Litter, Cow.pat, Rock))) %>% #remove extraneous variables
  ...
  rename(Lat.ven = Other.lathyrus,
         Rub.ida = Rub.ida1,
         Cir.und = Cir.umb) %>% #correct incorrect names
...
biomass23 <- read.csv("data/August 2023 aboveground biomass & litter weights.csv") %>%
  ...
  dplyr::select(Plot, AB.biomass, Productivity)
str(biomass23)
...
kin21 <- kin21 %>%
  ...
  dplyr::select(!c(Date.Collected, Block, Waypoint, Treatment, Bare.Ground, Litter, moss, Cow.pat, X)) %>% #remove extraneous variables
  rename(Hel.hoo2 = Heli.hoo) %>%
  mutate(Hel.hoo1 = Hel.hoo + Hel.hoo2,
         Ery.che1 = Ery.che + rough.mustard) %>% #collapse double observations
  dplyr::select(!c(Hel.hoo, Hel.hoo2, fuzz.auricle.grass, slender.wheat.grass, Ery.che, rough.mustard)) %>% #remove double and unidentified observations
...
write_rds(richness21, "output/richness21.rds")
...
kin23 <- kin23 %>% dplyr::select(order(colnames(.))) #put columns back in alphabetical order
kin21[not_in_kin21] <- 0
kin21 <- kin21 %>% dplyr::select(order(colnames(.)))
...
##From function documentation: When B > C, the site has lost species or abundances-per-species between time 1 and time 2; 
