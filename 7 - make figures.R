...
richness.gains.plot <- ggplot(gain.loss.new, aes(x = Richness, y = spp.gains)) +
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  labs(y = " ",
       x = " ") +
  theme_classic(base_size = 20) +
  annotate("text", x = -Inf, y = Inf, label = bquote(bold("p = 0.046")), hjust = -0.1, vjust = 1.1, size = 5)
richness.gains.plot

FRic.gains.plot <- ggplot(gain.loss.new, aes(x = FRic, y = spp.gains)) +
  geom_point() + 
  labs(y = " ",
       x = " ") +
  theme_classic(base_size = 20) +
  annotate("text", x = Inf, y = -Inf, label = bquote(bold("p = 0.486")), hjust = 1.1, vjust = -0.5, size = 5)
FRic.gains.plot
...
richness.losses.plot <- ggplot(gain.loss.new, aes(x = Richness, y = spp.losses)) +
  geom_point() + 
  labs(y = " ",
       x = " ") +
  theme_classic(base_size = 20) +
  annotate("text", x = Inf, y = -Inf, label = bquote(bold("p = 0.428")), hjust = 1.1, vjust = -0.5, size = 5)
richness.losses.plot

FRic.losses.plot <- ggplot(gain.loss.new, aes(x = FRic, y = spp.losses)) +
  geom_point() + 
  labs(y = " ",
       x = " ") +
  theme_classic(base_size = 20) +
  annotate("text", x = Inf, y = -Inf, label = bquote(bold("p = 0.832")), hjust = 1.1, vjust = -0.5, size = 5)
FRic.losses.plot
...
richness.turnover.plot <- ggplot(gain.loss.new, aes(x = Richness, y = turnover)) +
  geom_point() + 
  labs(y = " ",
       x = " ") +
  theme_classic(base_size = 20) +
  annotate("text", x = Inf, y = -Inf, label = bquote(bold("p = 0.474")), hjust = 1.1, vjust = -0.5, size = 5)
richness.turnover.plot

FRic.turnover.plot <- ggplot(gain.loss.new, aes(x = FRic, y = turnover)) +
  geom_point() + 
  labs(y = " ",
       x = " ") +
  theme_classic(base_size = 20) +
  annotate("text", x = Inf, y = -Inf, label = bquote(bold("p = 0.736")), hjust = 1.1, vjust = -0.5, size = 5)
FRic.turnover.plot
...
height.biomass.plot <- ggplot(gain.loss.new, aes(x = Max.Height, y = Productivity)) +
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  labs(y = ' ',
...
SLA.biomass.plot <- ggplot(gain.loss.new, aes(x = SLA, y = Productivity)) +
  geom_point() + 
  labs(y = " ",
       x = "CWM SLA (cm\u00B2/g)") +
  theme_classic(base_size = 20) + 
...
RTD.biomass.plot <- ggplot(gain.loss.new, aes(x = RTD, y = Productivity)) +
  geom_point() + 
  labs(y = " ",
       x = "CWM RTD (mg/cm\u00B2)") +
  theme_classic(base_size = 20) + 
...
richness.biomass.plot <- ggplot(gain.loss.new, aes(x = Richness, y = Productivity)) +
  geom_point() + 
  labs(y = " ",
       x = "2021 richness") +
  theme_classic(base_size = 20) +
  annotate("text", x = Inf, y = -Inf, label = bquote(bold("p = 0.126")), hjust = 1.1, vjust = -0.5, size = 5)
richness.biomass.plot

FRic.biomass.plot <- ggplot(gain.loss.new, aes(x = FRic, y = Productivity)) +
  geom_point() + 
  labs(y = " ",
       x = "FRic") +
  theme_classic(base_size = 20) +
  annotate("text", x = Inf, y = -Inf, label = bquote(bold("p = 0.372")), hjust = 1.1, vjust = -0.5, size = 5)
FRic.biomass.plot

nutrients.biomass.plot <- ggplot(nutrients.for.plots, aes(x = Nutrients, y = mean.productivity, fill = Nutrients)) +
  geom_col() +
  geom_errorbar(aes(ymin=mean.productivity-se.productivity, ymax=mean.productivity+se.productivity), position = "dodge") + 
  labs(x = "Nutrient treatment",
       y = expression("Productivity (g/m"^2*")")) +
  scale_y_continuous(labels = label_number(accuracy = 0.1)) +
  scale_fill_manual(values = c("#1F618D", "#DAF7A6")) +
  theme_classic(base_size = 20) + 
  annotate("text", x = Inf, y = Inf, label = bquote(bold("p < 0.001")), hjust = 1, vjust = 1, size = 5) + 
  annotate("text", x = 1, y = nutrients.for.plots$mean.gains[1] + 500, label = "a", size = 6) +
  annotate("text", x = 2, y = nutrients.for.plots$mean.gains[2] + 420, label = "b", size = 6)
nutrients.biomass.plot

light.biomass.plot <- ggplot(light.for.plots, aes(x = Light, y = mean.productivity, fill = Light)) +
  geom_col() +
  geom_errorbar(aes(ymin=mean.productivity-se.productivity, ymax=mean.productivity+se.productivity), position = "dodge") + 
  ...
  annotate("text", x = 1, y = light.for.plots$mean.productivity[1] + 70, label = "a", size = 6) +
  annotate("text", x = 2, y = light.for.plots$mean.productivity[2] + 50, label = "b", size = 6) +
  annotate("text", x = 3, y = light.for.plots$mean.productivity[3] + 50, label = "ab", size = 6)
light.biomass.plot

#### put it together ####
traits.resources.multiplot <- cowplot::plot_grid(
  nutrients.gains.plot + theme(legend.position = "none"), light.gains.plot + theme(legend.position = "none"), richness.gains.plot, FRic.gains.plot, height.gains.plot, SLA.gains.plot, RTD.gains.plot, 
  nutrients.losses.plot + theme(legend.position = "none"), light.losses.plot + theme(legend.position = "none"), richness.losses.plot, FRic.losses.plot, height.losses.plot, SLA.losses.plot, RTD.losses.plot, 
  nutrients.turnover.plot + theme(legend.position = "none"), light.turnover.plot + theme(legend.position = "none"), richness.turnover.plot, FRic.turnover.plot, height.turnover.plot, SLA.turnover.plot, RTD.turnover.plot, 
  nutrients.biomass.plot + theme(legend.position = "none"), light.biomass.plot + theme(legend.position = "none"), richness.biomass.plot, FRic.biomass.plot, height.biomass.plot, SLA.biomass.plot, RTD.biomass.plot, 
  nrow = 4,
  labels = 'auto')
